...
void setup() {
  ...
  pinMode(MOTOR_AIN2, OUTPUT); pinMode(MOTOR_AIN1, OUTPUT);
  pinMode(MOTOR_BIN1, OUTPUT); pinMode(MOTOR_BIN2, OUTPUT);
  Control_Motores(0, 0);
  pinMode(SENSOR_IN_P1, INPUT); pinMode(SENSOR_IN_P2, INPUT);
  pinMode(LED1, OUTPUT); pinMode(LED2, OUTPUT);
  pinMode(DIPSW2, INPUT); pinMode(DIPSW3, INPUT); pinMode(DIPSW4, INPUT);
  Serial.begin(9600);
  ...
  bandera_inicio = 1;
  switch (dsw) {
    case 0: { //ESPERAR DE FRENTE --2 3 4 OFF
        control_md =  30;
        control_mi =  30;
        timeout_str = 2000;
    ...
    case 1: { //OPOENTE DE DERECHA DIAGONAL --2 ON 3 4 OFF
    ...
    case 2: { //OPOENTE DE ATRAS CENTRO DE PISTA - GIRO DE UNA --2 OFF 3ON 4 OFF
    ...
    case 3: { //OPOENTE DE DERECHA GIRO COMPLETO --2 ON 3ON 4 OFF
        control_md = -VELOCIDAD_ATAQUE;
        control_mi =  VELOCIDAD_ATAQUE;
        timeout_str = 2000;
    ...
    case 5: { //OPOENTE DE FRENTE EN CENTRO DE PISTA --2 ON 3 OFF 4 ON
        control_md =  VELOCIDAD_ATAQUE;
        control_mi =  VELOCIDAD_ATAQUE;
        timeout_str = 2000;
        opnente_det = 110; //OPONENETE A IZQUIERDA
    ...
    case 6: { //OPOENTE DE IZQUIERDA GIRO TOTAL --2 OFF 3 ON 4 ON
        control_md = -VELOCIDAD_ATAQUE;
        control_mi =  VELOCIDAD_ATAQUE;
        timeout_str = 2000;
        opnente_det = 1000; //OPONENETE A IZQUIERDA
    ...
    case 7: { //OPONENTE ATRAS EN BORDE DE PISTA MINISUMO--2 3 4 ON
        control_md = -VELOCIDAD_ATAQUE;
        control_mi = -VELOCIDAD_ATAQUE;
        timeout_str = 2000;
        opnente_det = 1000; //OPONENETE A IZQUIERDA
...
void loop() {
  ...
  if (a_ubicacion != -1) {
    ...
    else if (a_ubicacion > 0) { // entre 100 y 0 -- oponente centro derecha
      control_md = -VELOCIDAD_ATAQUE / 4;
      control_mi =  VELOCIDAD_ATAQUE;
    ...
    } else if (a_ubicacion < 0) { // entre -100 y 0 -- oponente centro izquierda
      control_md = VELOCIDAD_ATAQUE;
      control_mi = -VELOCIDAD_ATAQUE / 4;
      opnente_det = 100; //0100 --SENSORES
    } else { //OPONENTE AL CENTRO
      control_md = 250;
      control_mi = 250;
  ...
  */
  ...
  ///////////////////////////////////////////////////////////////////////
    ...
    if (opnente_det == 1 || opnente_det == 10){
      ...
      control_md =  VELOCIDAD_BUSQUEDA;
      control_mi = -VELOCIDAD_BUSQUEDA;
    }else{ //OPONENTE AL CENTRO --  LO PERDI -- BUSCO AL LADO -- NO DE FRENTE
      control_md = VELOCIDAD_BUSQUEDA;
      control_mi = -VELOCIDAD_BUSQUEDA;
      opnente_det = 100;
    }
...
int Leer_Sensor_Piso(int pin, int cal) {
  int PinState; int cnt; int sensor;
  pinMode(pin, OUTPUT);
